...
- Mobile-QA: gekoppeltes-Testgerät-Harness für gespeicherte Sitzungen; deaktiviert die App-eigene Face-ID-Sperre ausschließlich während des Smoke-Laufs, kann eine leere Queue als Abschlussbedingung verlangen und stellt die ursprüngliche Einstellung zuverlässig wieder her.
...
- Mobile: **Erfassungs-Screens (Fallwild, Ansitze, Meldungen) auf ein gemeinsames „Erfassen | Bestand"-Muster umgebaut.** Ein full-width-Segmented-Control im Hero trennt das Eingabeformular vom Bestand (Liste/Karte) — der Bestand ist mit einem Tap erreichbar statt durch langes Scrollen. Der Liste/Karte-Umschalter sitzt jetzt direkt über der Karte. Pro Formular nur noch **ein** „Speichern"-Button; „Warteschlange senden" wandert in den Offline-Vormerkungen-Block (nur sichtbar, wenn etwas wartet) mit erklärendem Hinweis zur automatischen Synchronisierung. Fallwild: Foto-Aufnahme/Mediathek als kompakte Icon-Kacheln, Entwickler-Mikrocopy entfernt, Bergungsstatus lesbar („Geborgen" statt „geborgen"). „Aktualisieren"-Button entfällt zugunsten von Pull-to-Refresh am ganzen Screen; `ViewToggle` um eine `block`-Variante erweitert. OTA `0.1.0 · 2026-06-13.16`.
- Web: **lesbare Rollen-Labels überall** — neues geteiltes `apps/web/src/lib/labels.ts` (`formatRoleLabel`) ersetzt drei identische Lokal-Kopien (Shell, Kontakte, Mitglieder) und macht fünf rohe Ausgaben lesbar („Schriftführung · GF-12" statt „schriftfuehrer · GF-12" in Sitzungs-Teilnehmerlisten und Aufgaben-Zuweisungen) (PR #172).
- Einrichtungs-Zustand als deutscher Begriff: „Wartung fällig" statt `wartung-faellig` in Web-Status-Pill, Karten-Infofenster, Mobile-Listenkarte und Karten-Pin-Subtitle — geteilte Helper `formatEinrichtungZustand` (Web `lib/labels.ts`, Mobile `lib/format.ts`) (PR #172).
- Mobile: **Warteschlangen-Mikrocopy vervollständigt** — letzte „Queue"-Strings eingedeutscht (Heute-Sync-Meldung, Fallwild-Feedback, Queue-Badge-VoiceOver-Labels); Web-Protokoll-Detail zählt grammatisch korrekt („1 gespeicherte Version") (PR #172).
- Mobile: **Erfolgs-Haptik beim Speichern** von Ansitz, Fallwild, Reviermeldung und Aufgabenstatus; **Pull-to-Refresh auf Protokolle** (vorher keine Aktualisierung ohne Tab-Wechsel) und brand-getintete RefreshControls auf Ansitze/Fallwild/Revierarbeit; OTA `0.1.0 · 2026-06-10.14` (PR #172).
...
- E2E-Suite repariert (war seit dem Setup-Wizard-Umbau #74 und dem Umlaut-Pass still rot): public-web testet den vierschrittigen Wizard inkl. realem Abschluss von Schritt 1, Rollen-Guard-Test asserted die `keine-berechtigung`-URL + Forbidden-Banner, stale Dashboard-Headings/Umlaut-Texte korrigiert, Smoke-Setup-Strings aktualisiert; `global.setup` startet Docker-Postgres nur noch, wenn keiner erreichbar ist (Worktree-Kompatibilität); macOS-Snapshot-Baselines ergänzt (bisher nur win32) — voller lokaler Lauf 41 passed / 0 failed (PR #171).
